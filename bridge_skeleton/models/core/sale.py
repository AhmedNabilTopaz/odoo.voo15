...
class SaleOrder(models.Model):
    ...
    def action_confirm(self):
        ...
        orders = self.filtered(lambda rec: rec.ecommerce_channel in ecomm_cannels)
        date_order_info = {order.id: order.date_order for order in orders}
        res = super(SaleOrder, self).action_confirm()
        for order in orders:
            order.write({'date_order': date_order_info.get(order.id, fields.Datetime.now())})
        return res
