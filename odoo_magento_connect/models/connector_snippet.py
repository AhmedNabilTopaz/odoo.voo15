...
class ConnectorSnippet(models.TransientModel):
    ...
    def _get_ecomm_extensions(self):
        res = super(ConnectorSnippet, self)._get_ecomm_extensions().__add__([('magento2', 'Magento 2')])
        return res
    ...
    def create_magento2_connector_mapping(self, model, data, connection):
    ...
    def update_magento2_connector_mapping(self, model, data, connection):
        if connection.get('status', False):
            ecomm_id, odoo_id, name = data.get('ecomm_id', 0), data.get('odoo_id', 0), data.get('name', 0)
