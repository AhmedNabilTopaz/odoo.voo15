...
class ConnectorInstance(models.Model):
    ...
    manage_multi_source = fields.Boolean(
        string='Manage Multi Source Inventory',
        default=False,
        help="If Enable, Odoo stock will update from respective odoo location to mapped Magento Source.")
    ...
    def test_magento2_connection(self):
        token = 0
        ...
        token = self.create_magento2_connection()
        if token.get('token', ''):
            self.token = str(token.get('token', ''))
            self.set_default_magento2_website(
    ...
    def _create_magento2_connection(self):
        ...
        if instance_obj:
            token_generation = instance_obj.create_magento2_connection()
    ...
    def create_magento2_connection(self, vals={}):
        text, token = '', ''
    ...
    def set_default_magento2_website(self, url, token):
        for obj in self:
            ...
            if not store_id:
                store_info = self.with_context(
                    ctx)._fetch_magento2_store(url, token)
                if not store_info:
                    raise UserError(
                        _('Error!\nMagento Default Website Not Found!!!'))
        return True

    def _fetch_magento2_store(self, url, token):
        store_info = {}
        store_obj = self.env['magento.store.view']._get_magento2_store_view(url, token)
    ...
    def fetch_connection_info(self, vals):
        ...
        if vals.get('magento_url'):
            active_connections = self.search([('active', '=', True), ('ecomm_type', '=', vals.get('ecomm_type', False))])
            ...
            if is_multi_mob_installed:
                ...
                for connection_obj in active_connections:
                    return connection_obj.read(
                        ['language', 'category', 'warehouse_id', 'manage_multi_source'])[0]
        return False
